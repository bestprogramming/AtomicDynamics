...
{
    public class Scene(Simulation simulation) : IDisposable
    {
        public string Title = "Atomic Dynamics";
        public int Width = 1920;
        public int Height = 1080;

        private static bool showUcs = true;
        private readonly Ucs ucs = new((float)(BohrRadius * Zoom));
        ...
        private void KeyDown(IWindow window, GL gl, IKeyboard keyboard, Key key, int arg3)
        {
            ...
            else if (key == Key.F2)
            {
                foreach(var p in simulation.Particles)
                {
                    p.Trace.Enabled = !p.Trace.Enabled;
                    p.Trace.VerticesList = [];
                }
            }
            else if (key == Key.F3)
            {
